...
export const metadata: Metadata = {
  title: "تعرفه‌ها",
  description:
    "تعرفه‌ها و بسته‌های ثبت‌نام باشگاه بانوان ابر سفید در کرج — جلسهٔ آشنایی رایگان، تک‌جلسه، بسته‌های ماهانه و کلاس خصوصی، همراه با شرط‌های هرکدام. ثبت‌نام تلفنی یا حضوری.",
...
export default function PricingPage() {
  return (
    <PageShell
      ...
      lead="هر بسته‌ای که انتخاب کنید، قدم اول یکی است: یک تماس کوتاه تا با هم ببینیم کدام کلاس و کدام ساعت به شما می‌خورد."
    >
      {planGroups.map((group, index) => (
        ...
        >
          ...
          {/*
            شمار ستون‌ها باید شمار کارت‌ها را بشمارد، وگرنه کارت آخر تنها
            در یک ردیف می‌ماند و مثل چیزی که جا افتاده دیده می‌شود.
            گروه سه‌تایی عمداً از مرحلهٔ دوستونه رد می‌شود: در ۹۰۰ پیکسل،
            «۸ جلسه» و «۱۲ جلسه» کنار هم می‌نشستند و «نامحدود» تنها می‌ماند.
            حالا از md یک‌راست سه‌ستونه می‌شود — در ۷۶۸ پیکسل هر کارت ۲۱۹
            پیکسل عرض دارد که برای قیمت و شرط‌ها اندازه‌گیری شده و کافی است.
          */}
          <div
            className={`grid gap-6 ${
              group.plans.length === 3 ? "md:grid-cols-3" : "sm:grid-cols-2"
            }`}
          >
      ...
      <Section className="border-t border-line">
        {/*
          لید حالا JSX است نه رشته، پس شماره همان <PhoneNumber> بقیهٔ سایت
          است و جداسازی دوجهته و nowrap را با خودش می‌آورد.
          جداکنندهٔ دستی LRI…PDI که اینجا بود حذف شد: جهت را درست می‌کرد ولی
          جلوی شکستن شماره وسط دو گروه رقم را نمی‌گرفت — و کاراکتر نامرئی
          داخل رشته چیزی است که دفعهٔ بعد کسی سهواً پاکش می‌کند.
        */}
        <SectionHeading
          title="قدم بعدی"
          lead={
            <>
              ثبت‌نام تلفنی یا حضوری انجام می‌شود — شمارهٔ استودیو{" "}
              <PhoneNumber />. اگر هنوز تصمیم نگرفته‌اید، این دو صفحه معمولاً
              باقی سؤال‌ها را جواب می‌دهند.
            </>
          }
        />
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          ...
          >
            ...
            <span className="mt-3 text-sm leading-loose text-muted">
              چه بپوشم، مت لازم است یا نه، دیر رسیدن چه می‌شود — جواب چیزهایی
              که پیش از اولین جلسه ذهن‌تان را مشغول می‌کند.
            </span>
          </Link>
          ...
          >
            ...
            <span className="mt-3 text-sm leading-loose text-muted">
              ساعت کلاس‌ها را ببینید و مطمئن شوید بسته‌ای که انتخاب می‌کنید با
              روزهای آزادتان جور درمی‌آید.
            </span>
          </Link>
        </div>
      </Section>
