...
export const metadata: Metadata = {
  title: "کلاس‌ها",
  description:
    "شش سبک یوگای باشگاه بانوان ابر سفید در کرج — هاتا، وینیاسا، یین، آشتانگا، هوت یوگا و رستوراتیو. در هر کلاس چه اتفاقی می‌افتد، برای چه کسی مناسب است و کدام مربی آن را می‌گیرد.",
...
const suitedFor: Record<StyleId, string> = {
  hatha:
    "اگر اولین بارتان است، یا مدتی ورزش نکرده‌اید و می‌خواهید بدون فشار برگردید. هاتا وقت می‌گذارد تا بدن‌تان زبان تمرین را یاد بگیرد؛ هیچ حرکتی سریع رد نمی‌شود.",
  vinyasa:
    "اگر وضعیت‌های پایه را می‌شناسید و دنبال کلاسی هستید که نفس‌تان را بالا بیاورد و بدن‌تان را گرم نگه دارد. برای کسانی که در تمرین‌های کند حوصله‌شان سر می‌رود.",
  yin: "اگر روزتان پشت میز می‌گذرد و شب‌ها کمر، لگن و پشت پاهایتان گرفته است. یین آمادگی جسمانی نمی‌خواهد؛ حوصله می‌خواهد.",
  ashtanga:
    "اگر با نظم و تکرار جان می‌گیرید و دوست دارید پیشرفت‌تان را هفته‌به‌هفته روی یک توالی ثابت اندازه بگیرید. برای شروع از صفر سنگین است؛ چند ماه هاتا یا وینیاسا پیش از آن کار را خیلی راحت‌تر می‌کند.",
  hot: "اگر بدن‌تان دیر گرم می‌شود و انعطاف به‌سختی به دست‌تان می‌آید. اگر فشار خون پایین، بارداری یا مشکل قلبی دارید، پیش از ثبت‌نام با ما و با پزشک‌تان مشورت کنید.",
  restorative:
    "اگر خسته‌اید، کم خوابیده‌اید یا دورهٔ پرفشاری را می‌گذرانید. سطح آمادگی مهم نیست؛ تنها کاری که می‌کنید این است که بگذارید بدن‌تان روی ابزار رها شود.",
};
...
export default function ClassesPage() {
  return (
    <PageShell
      eyebrow="کلاس‌ها"
      title="شش سبک، شش ریتم"
      lead="لازم نیست از قبل بدانید دنبال چه هستید. هر سبک را با ریتم، مخاطب و مربی‌اش آورده‌ایم تا ببینید کدام با وقت و حال این روزهای شما جور درمی‌آید."
    >
      <Section>
                    ...
                    <div className="mt-6">
                      ...
                      {teaching ? (
                        ...
                        <p className="mt-3 text-sm leading-loose text-muted">
                          این سبک فعلاً در برنامهٔ هفتگی جایی ندارد. اگر
        ...
        <div className="mt-16 rounded-2xl border border-line bg-surface p-6 text-center sm:mt-20 sm:p-10">
          ...
          <p className="mx-auto mt-4 max-w-xl text-base leading-loose text-muted">
            ...
            جور است. اگر تازه شروع می‌کنید، «جلسهٔ آشنایی» رایگان است؛ همان‌جا
            با مربی حرف می‌زنید و با هم سبک مناسب‌تان را پیدا می‌کنیم.
          </p>
